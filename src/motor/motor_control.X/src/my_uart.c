...
BOOL sendUARTarr(unsigned char length, unsigned char* data){
    if(length > MSGLEN){
        // This will be populated when we implement error-checking
        return false;
    }
    // Send message containing data to be transmitted to interrupt handler
...
BOOL sendUARTbyte(unsigned char data){
    unsigned char temp[1];
    temp[0] = data;
    // Send message containing data to be transmitted to interrupt handler
    signed char retval = ToMainLow_sendmsg(1, MSGT_UART_TX, (void *)temp);
    if (retval < 0) {
        // This will be populated when we implement error-checking
        return false;
    }
